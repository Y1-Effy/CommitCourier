...
- **At-rest encryption of signing secrets (precondition).** Signing secrets must be encrypted at rest via one of: DB disk encryption, column encryption, or the built-in `cipher`. Pass `cipher` to `createRelay` — `createAesGcmCipher(key)` (WebCrypto AES-256-GCM) or your own `SecretCipher` over a KMS/Vault — and `webhook_outbox.secret_snapshot` / `webhook_endpoints.secret` are stored as ciphertext (a versioned `ccsec.v1.` envelope) and decrypted only in memory just before signing. Tampering is detected by GCM authentication. Without a `cipher`, `createRelay` warns at startup unless you set `unsafeAllowPlaintextSecrets: true` to acknowledge that the DB handles at-rest encryption.
...
- **Encryption-key management.** At-rest encryption of signing secrets is a precondition (DB disk encryption, column encryption, or `cipher`). When you use the `cipher` option above, storing, distributing, and rotating the encryption key is your responsibility. Without a `cipher`, signing secrets are stored as written by your application — `createRelay` warns at startup and encrypting them at rest is your database's responsibility; acknowledge with `unsafeAllowPlaintextSecrets: true`.
