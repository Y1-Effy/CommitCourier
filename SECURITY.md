...
1. Open a new advisory: **https://github.com/Y1-Effy/RelayBox/security/advisories/new** (or go to the repository's **Security** tab → **Report a vulnerability**).
...
Relaybox is pre-release. Until `1.0.0`, only the latest `0.x` release receives security fixes.
...
Relaybox handles security-sensitive concerns, so it helps to be explicit about what the library protects and what remains the integrator's responsibility.

### What Relaybox protects
...
- **Receiver-side verification and idempotency.** Relaybox provides at-least-once delivery plus an idempotency key, but verifying the signature and de-duplicating events is the receiver's responsibility.
