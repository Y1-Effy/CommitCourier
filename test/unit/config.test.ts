...
describe("config.resolveConfig defaults", () => {
  it("fills the documented defaults (00-overview section 6)", () => {
    ...
    expect(cfg.circuitBreaker).toEqual({ failureThreshold: 0, cooldownMs: 0 });
  });
  ...
  it("accepts a positive circuitBreaker.cooldownMs", () => {
    const cfg = resolveConfig({ circuitBreaker: { failureThreshold: 5, cooldownMs: 60_000 } });
    expect(cfg.circuitBreaker.cooldownMs).toBe(60_000);
  });
...
describe("config.resolveConfig validation (fail-fast)", () => {
  it.each([
    ...
    [
      "baseMs Infinity",
      { retry: { baseMs: Number.POSITIVE_INFINITY, capMs: Number.POSITIVE_INFINITY } },
    ],
    ["baseMs fractional", { retry: { baseMs: 1_000.5 } }],
    ["capMs <= 0", { retry: { capMs: 0 } }],
    ["capMs Infinity", { retry: { capMs: Number.POSITIVE_INFINITY } }],
    ["capMs fractional", { retry: { capMs: 3_600_000.5 } }],
    ...
    ["timeoutMs Infinity", { delivery: { timeoutMs: Number.POSITIVE_INFINITY } }],
    ["timeoutMs fractional", { delivery: { timeoutMs: 15_000.5 } }],
    ["bodySnippetBytes <= 0", { delivery: { bodySnippetBytes: 0 } }],
    ["bodySnippetBytes Infinity", { delivery: { bodySnippetBytes: Number.POSITIVE_INFINITY } }],
    ["keepAliveTimeoutMs <= 0", { delivery: { keepAliveTimeoutMs: 0 } }],
    ["keepAliveTimeoutMs Infinity", { delivery: { keepAliveTimeoutMs: Number.POSITIVE_INFINITY } }],
