...
1. 新しいアドバイザリを作成する：**https://github.com/Y1-Effy/RelayBox/security/advisories/new**（または、リポジトリの **Security** タブ → **Report a vulnerability**）。
...
Relaybox はプレリリースです。`1.0.0` までは、最新の `0.x` リリースのみがセキュリティ修正の対象です。
...
Relaybox はセキュリティ機微な責務を扱うため、ライブラリが守るものと、利用者（インテグレータ）の責務として残るものを明示します。
...
- **受信側の検証と冪等処理。** Relaybox は at-least-once 配信と idempotency key を提供しますが、署名検証とイベントの重複排除は受信側の責務です。
