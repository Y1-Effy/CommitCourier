...
両方を兼ねるのは CommitCourier だけです：Outbox 行を**あなたのトランザクション内**で書き（dual-write 不整合が原理的に起きない）、かつ **webhook-grade の HTTP 配信**（署名・リトライ・DLQ・台帳・SSRF）まで運びます。SaaS や Redis 型はローカルトランザクションに入れず、ブローカー Outbox 系は相乗りできてもメッセージブローカーで止まります。
