...
> 🎬 **[ライブデモを試す](https://commitcourier-demo.xvps.jp/)** — 実際の Postgres 上で動いている稼働インスタンスです。トランザクション内でイベントを enqueue し、dispatcher を走らせて、署名・リトライ・DLQ・リプレイが起きる様子をそのまま観察できます。配信先を不安定な受信側（遅延・タイムアウト・`500`・恒久失敗）に切り替えれば、各経路の挙動も確認できます。インストール不要。
...
const relay = await createRelay({
  store,
  // 最初から logger を注入する：既定は no-op のため、未注入だと通常の配信失敗・リトライが
  // 沈黙する。詳しくは [ロギングと可観測性](#ロギングと可観測性) を参照。
  ...
  // maxPayloadBytes: 1_048_576, // 任意・既定オフ：enqueue する payload のサイズ上限
});
```
...
- **エンドポイント単位のカスタムヘッダ** — 署名とは別に独自認証を要する受信側向けに、追加ヘッダ（例：`authorization: Bearer …`）を付与できる。値は秘密情報として扱い、`cipher` で保管時暗号化し、台帳では redact する。
...
> **クラッシュループ（毒行）。** `attempts` は配信**試行が完了してその結果を記録した時だけ**増分されます。その前にプロセスがクラッシュ（OOM・`SIGKILL` 等）した場合は `attempts` が据え置かれ、回収は retry 予算を消費せずに行を `pending` へ戻します。これは意図的で、一過性のインフラ障害が試行を浪費したりイベントを失わせたりしないためです。トレードオフとして、**毎回ワーカーを落とす**行は DLQ に落ちず `reclaimAfterMs` ごとに無限に再配信され得ます。通常の配信失敗（HTTP エラー・タイムアウト・配信内例外）は必ず試行を記録し `maxAttempts` にカウントされます。免除されるのは配信中のハードクラッシュのみです。そうした行を止めたい場合は、繰り返し回収される兆候（古い `created_at` のまま `pending`/`in_flight`）を監視し `relay.cancel(id)` してください。
...
|            | `maxPayloadBytes`    | _(無効)_              | enqueue する payload のシリアライズ後 UTF-8 バイト長の任意上限。超過時は `ENQUEUE_INVALID_PAYLOAD` で拒否。シリアライズ可能性は常時検証。 |
...
エンドポイントが `cooldownMs` 以上 disable され続けると、dispatcher は配信を half-open 試行として通します。成功すれば再 active 化（＋ counter リセット）、失敗すれば cooldown を再武装し次の試行はさらに `cooldownMs` 待ちます。cooldown 内は HTTP 試行を一切行いません。breaker でも `410 Gone` でも disable された任意の登録エンドポイントに適用され、`cooldownMs: 0`（既定）は手動回復のままにします。

> **試行の同時性。** この試行が「1 回だけ」になるのは `ordering: "per-endpoint"`（かつ単一 dispatcher インスタンス）のときだけです（エンドポイントあたり in-flight 1 本に直列化）。既定の `ordering: "none"` では、その時点で due な同一エンドポイント宛の `pending` 行が同時に試行として通るため、復旧中のまだ脆弱なエンドポイントへ一斉にバーストし得ます。回復プローブを単一リクエストにしたい場合は `"per-endpoint"` ordering を使ってください。

### エンドポイント単位のカスタムヘッダ

受信側が署名とは別に独自の認証を要求することがあります（`x-api-key` を求める API Gateway、`authorization` を求める ingress、テナントのルーティングヘッダなど）。登録エンドポイントに紐付ければ、そのエンドポイント宛の全配信に付与されます。

```ts
await relay.endpoints.register({
  url: "https://api.customer.example/hooks/orders",
  secret: "whsec_...",
  customHeaders: { authorization: "Bearer cust_live_abc123" },
});
```

ヘッダ名は小文字化されます。読み出しは `relay.endpoints.get(endpointId)`。`relay.endpoints.list()` には**意図的に出しません**（こちらは secret-free な面のため。後述）。

> **カスタムヘッダは署名の対象外です。** 署名が覆うのは Standard Webhooks 準拠で `id.timestamp.body` のみ。受信側は `webhook-signature` からヘッダの真正性を導けないため、カスタムヘッダは通常の意味での bearer 認証情報であり、その安全性は運んでいる TLS 接続に依存します。送信先は `https://` に限定してください。

**値は秘密情報として扱われます。** `cipher` 設定時は保管時暗号化されます。**値ごと**に暗号化するので、DB 上でヘッダ**名**は可読のままで、どのヘッダを送るのかを確認できます。`cipher` 未設定なら `secret` と同様に平文で保存され、起動時の PLAINTEXT 警告の対象になります。

配信台帳では**ヘッダ名は残し、値はすべて** `[redacted]` に伏せます。認証情報を溜め込まずに、失敗配信でどのヘッダを送ったかは追えます。

```jsonc
// relay.attempts({ outboxId })[0].requestHeaders
{
  "webhook-id": "018f…",
  "webhook-signature": "v1,k8s…",
  "content-type": "application/json",
  "authorization": "[redacted]",
}
```

検証は配信時の暗黙の drop ではなく、`register` ／ `update` 時の fail-closed（`INVALID_ARGUMENT`）です。

| ルール       | 内容                                                                                                                                   |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------- |
| 予約名       | `webhook-*` 名前空間全体、`content-type`、`idempotency-key`、hop-by-hop / framing 系（`host`、`connection`、`transfer-encoding` 等）。 |
| 値の文字種   | 印字可能 ASCII とタブのみ。CR/LF（ヘッダインジェクション）・NUL・制御文字・非 ASCII は拒否。                                           |
| 値の形       | 空値と前後の空白は拒否（trim して直したりはしない）。                                                                                  |
| 大文字小文字 | 名前は小文字化。小文字化後に衝突する 2 つの綴りは、片方が黙って勝つのではなく拒否。                                                    |
| サイズ       | 16 本まで、直列化して 8 KiB まで。                                                                                                     |

`delivery.transport: "sink"` では使用できません（`register` ／ `update` が `CONFIG_INVALID`）。この場合リクエストは sink／SaaS 側が組み立てるため、ヘッダはそちらで設定してください。
...
> **アイドル時のポーリング負荷（多インスタンス）。** 既定の `pollIntervalMs`（1 秒）では、各 Dispatcher はアイドル時も毎秒約 1 回 claim クエリを発行するため、_N_ インスタンスなら毎秒 _N_ クエリが `webhook_outbox` に当たります。各クエリは安価（部分インデックス `ix_outbox_due WHERE status = 'pending'` でスキャンは小さい）ですが、ゼロではありません。常駐 Dispatcher を多数走らせる場合は、下記のアクセラレータと長めの `pollIntervalMs`（例：`10_000`）を併用してください。wake が低遅延配信を担い、ポーリングは低速なフォールバックの安全網になります。
...
| code                      | 発生元                                                                                                        | 意味                                                                                                                           |
| ------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `CONFIG_INVALID`          | `createRelay`（起動時）／`endpoints.register`・`endpoints.update`                                             | 設定が不正（fail-fast）。エンドポイント系では `transport: "sink"` での `customHeaders` 指定。                                  |
...
| `INVALID_ARGUMENT`        | `list` / `get` / `cancel` / `replay` / `prune` / `endpoints.list` / `endpoints.register` / `endpoints.update` | 引数が不正（例：数値でない `cursor`、未知の `status`、uuid でない id、不正な `olderThan`、予約済み／不正な `customHeaders`）。 |
...
| `webhook_endpoints`         | 任意の登録エンドポイントレジストリ（登録エンドポイント運用のみ）。各エンドポイントの URL・署名 secret・カスタムヘッダを保持。 | 長期保持の設定。prune 対象外。                 |
...
- **大規模な既存 DB。** 新規導入時はテーブルが空なので DDL は一瞬です。将来のマイグレーションが既に巨大な `webhook_outbox` にインデックス追加や `ALTER` を行う場合、`migrate()` は通常の `CREATE INDEX`（`CONCURRENTLY` ではない＝`migrate()` のトランザクション内では使えないため）でインデックスを構築するため、構築中は書き込みロックを取ります。低トラフィックの時間帯に単一インスタンスから実行するか、デプロイ前に同等の `CREATE INDEX CONCURRENTLY` を手動で先行適用してください。
...
| import                                        | エクスポート                                                                                                                                                                                                                                                |
| --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `commitcourier`                               | `createRelay`、`createConsoleLogger`、`Relay`/`RelayInit` 型、`Store` ポート（および capability ロール `OutboxEnqueueStore` / `DispatchStore` / `EndpointStore` / `OutboxQueryStore` / `ReplayStore` / `MaintenanceStore` / `SchemaStore`）、全ドメイン型。 |
...
| **Beta** — minor で変更あり得る                           | 登録エンドポイント管理 API（エンドポイント単位のカスタムヘッダを含む）、回路遮断、登録エンドポイントキャッシュ、OpenTelemetry アダプタ、LISTEN/NOTIFY アクセラレータ、リプレイ、保持/削除、`cancel`、`doctor` CLI。 |
